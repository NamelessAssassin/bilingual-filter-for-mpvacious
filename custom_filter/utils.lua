...
-- Virtual path configuration mapping
local ROOT_PKG = "custom_filter"
local VIRTUAL_PATHS = {
    ["@ROOT"] = ROOT_PKG,
    ["@CONFIG"] = ROOT_PKG .. ".config",
    ["@MENU"] = ROOT_PKG .. ".menu"
}

--- Resolves virtual path symbols (e.g., @CONFIG) to actual package paths
--- @param path string
--- @return string
local function resolve_virtual_path(path)
    for symbol, real_path in pairs(VIRTUAL_PATHS) do
        if path:find(symbol, 1, true) then
            -- Replace symbol with the mapped package path
            return path:gsub(symbol, real_path)
        end
    end
    return path
end

--- Normalizes relative markers like ./ and ../ into absolute package paths
--- @param path string
--- @param current_pkg string
--- @return string
local function normalize_relative_path(path, current_pkg)
    local prefix = current_pkg:match("(.-)%.[^%.]+$") or ""

    if path:sub(1, 2) == "./" then
        return prefix .. "." .. path:sub(3)
    elseif path:sub(1, 3) == "../" then
        local parent_prefix = prefix:match("(.-)%.[^%.]+$") or ""
        return (parent_prefix ~= "" and parent_prefix .. "." or "") .. path:sub(4)
    end

    return path
end

--- Enhanced relative require function with virtual path support
--- @param path string: The path (supports "./", "../", or "@SYMBOL")
--- @param current_pkg string: The caller's '...' context
--- @return any: The loaded module
this.require_relative = function(path, current_pkg)
    if not current_pkg then
        error("Relative require requires the current package context ('...')")
    end

    local final_path = path

    -- Step 1: Handle virtual paths first (e.g., @CONFIG/settings)
    final_path = resolve_virtual_path(final_path)

    -- Step 2: Normalize relative notation if present
    final_path = normalize_relative_path(final_path, current_pkg)

    -- Clean up any double dots that might have been created
    final_path = final_path:gsub("%.%.+", "."):gsub("^%.", "")
